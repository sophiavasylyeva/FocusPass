...
class FocusPassWorkflowService {
  static final FocusPassWorkflowService _instance =
  ...
  /// Resolves the currently active child session. Tries the in-memory
  /// session first (set at login/auto-login within this app run), then
  /// falls back to restoring it from SharedPreferences (e.g. if this
  /// service is invoked from a fresh isolate/background context that
  /// hasn't gone through login in-process). Identity is ALWAYS parentUid +
  /// childId here — never the display name.
  Future<ChildSession?> _getCurrentSession() async {
  ...
  Future<AppAccessResult> handleAppAccessAttempt(String appName) async {
    ...
    print(
      'FocusPassWorkflowService: Today tasks - Total: ${todayTasks.length}, Pending: ${pendingTasks.length}, Completed: ${completedTasks.length}',
    );
    ...
    final usedSessions = await _getUsedSessionsToday(session);
    ...
    if (availableSessions <= 0) {
      ...
      print(
        'FocusPassWorkflowService: Access denied - need to complete $tasksNeeded more tasks (have completed ${completedTasks.length} tasks today)',
      );
      return AppAccessResult.denied(
        'Complete your educational task first to earn 15 minutes of screen time',
      );
    }

    // Step 3: Check if child has exceeded daily limit
    final totalUsedMinutesToday = await _getTotalUsedMinutesToday(session);
    final dailyLimitMinutes = await _getDailyLimitMinutes(session);
    ...
    if (totalUsedMinutesToday >= dailyLimitMinutes) {
      ...
      await _showDailyLimitBlockingOverlay(
        appName,
      ...
      print(
        'FocusPassWorkflowService: Access denied - daily limit reached ($totalUsedMinutesToday/${dailyLimitMinutes} minutes)',
      );
      return AppAccessResult.denied(
        'Daily screen time limit reached. Parental PIN required.',
      );
    }

    // Step 4: Grant access for exactly 15-minute session
    const sessionMinutes =
        15; // Each completed set of 5 tasks = exactly 15 minutes
    ...
    await _recordSessionUsed(session);

    print(
      'FocusPassWorkflowService: Access granted - 15 minutes session started',
    );
  ...
  Future<void> _startAppSession(String appName, int remainingMinutes) async {
    ...
    print(
      'FocusPassWorkflowService: 🚀 Starting session for $appName - $remainingMinutes minutes',
    );
    print(
      'FocusPassWorkflowService: Session timer will expire at ${DateTime.now().add(Duration(minutes: remainingMinutes))}',
    );
  ...
  Future<void> _cancelScheduledNotifications() async {
    try {
      ...
      print(
        'FocusPassWorkflowService: Cancelled any existing scheduled notifications',
      );
    } catch (e) {
      print(
        'FocusPassWorkflowService: Error cancelling scheduled notifications: $e',
      );
    }
  }
  ...
  Future<void> _scheduleSessionExpirationNotification(
    String appName,
    int minutes,
  ) async {
    try {
      ...
      print(
        'FocusPassWorkflowService: Scheduling session expiration notification for $appName at $expirationTime',
      );
      ...
      print(
        'FocusPassWorkflowService: Error scheduling session expiration notification: $e',
      );
    }
  }
  ...
  Future<void> _handleSessionExpired() async {
    if (_currentActiveApp == null) {
      print(
        'FocusPassWorkflowService: Session expired but no active app found',
      );
      return;
    }
    ...
    print(
      'FocusPassWorkflowService: ⏰ SESSION EXPIRED for $appName after exactly $duration minutes',
    );
    ...
    print(
      'FocusPassWorkflowService: Session expired notification and overlay sent - user must complete 5 more tasks',
    );
  ...
  Future<void> _showSessionExpiredOverlay(String appName) async {
    try {
      ...
      await platform.invokeMethod('showTimeExceededBlockingOverlay', {
        ...
        'title': '⏰ 15 Minutes Up!',
        'message':
            'Your 15-minute $appName session is over. Complete 5 more educational tasks to earn another 15 minutes!',
        'actionText': 'Complete 5 More Tasks',
        'actionType': 'session_expired',
      });
      print(
        'FocusPassWorkflowService: Session expired overlay triggered for $appName',
      );
    } catch (e) {
      print(
        'FocusPassWorkflowService: Error showing session expired overlay: $e',
      );
    }
  }

  /// Show blocking overlay when daily limit is reached - makes apps unclickable
  Future<void> _showDailyLimitBlockingOverlay(
    String appName,
    int usedMinutes,
    int limitMinutes,
  ) async {
    try {
      ...
      await platform.invokeMethod('showDailyLimitBlockingOverlay', {
        ...
        'message':
            'You have used ${usedHours}h of your ${limitHours}h daily limit for $appName. All screen time has been used up for today.',
      ...
      print(
        'FocusPassWorkflowService: Daily limit blocking overlay triggered for $appName ($usedMinutes/$limitMinutes minutes used)',
      );
    } catch (e) {
      print(
        'FocusPassWorkflowService: Error showing daily limit blocking overlay: $e',
      );
    }
  }
  ...
  Future<void> onEducationalTasksCompleted(ChildSession session) async {
    print(
      'FocusPassWorkflowService: Educational tasks completed for childId ${session.childId}',
    );
    ...
    print(
      'FocusPassWorkflowService: Tasks completed, earned $totalEarnedMinutes minutes',
    );
  }

  /// Handle parental PIN override
  Future<bool> handleParentalPinOverride(
    String appName,
    String enteredPin,
  ) async {
    ...
    if (isValidPin) {
      print(
        'FocusPassWorkflowService: Parental PIN verified, granting unlimited access to $appName',
      );
  ...
  /// Verify parental PIN. Uses the current session's parentUid directly —
  /// never re-derives it via a name-based lookup, so this can no longer
  /// resolve to a different family's parent PIN.
  ...
  /// Get learning to screen time ratio for child. Prefers
  /// `children[childId]`, falling back to the legacy
  /// `children[displayName]` entry — see [resolveChildRuleEntry].
  Future<double> _getLearningRatio(ChildSession session) async {
    try {
      ...
      if (data['applySameForAll'] == true) {
        ...
        return (entry?['ratio'] ?? 1.0).toDouble();
      }
    } catch (e) {
  ...
  /// Get used sessions count for today. Keyed by the stable childId rather
  /// than the mutable display name, so this counter can't collide across
  /// two children of the same parent sharing a name, and doesn't reset
  /// unexpectedly if the display name is edited mid-day.
  Future<int> _getUsedSessionsToday(ChildSession session) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final today = DateTime.now();
      final key =
          '${session.childId}_used_sessions_${today.year}_${today.month}_${today.day}';
  ...
  Future<void> _recordSessionUsed(ChildSession session) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final today = DateTime.now();
      final sessionsKey =
          '${session.childId}_used_sessions_${today.year}_${today.month}_${today.day}';
      final minutesKey =
          '${session.childId}_used_minutes_${today.year}_${today.month}_${today.day}';
      ...
      print(
        'FocusPassWorkflowService: Recorded session used for childId ${session.childId}. Total today: ${currentUsed + 1} sessions (${currentMinutes + 15} minutes)',
      );
    } catch (e) {
  ...
  Future<int> _getTotalUsedMinutesToday(ChildSession session) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final today = DateTime.now();
      final key =
  ...
  /// Get daily limit in minutes for child. Prefers `children[childId]`,
  /// falling back to the legacy `children[displayName]` entry — see
  /// [resolveChildRuleEntry].
  Future<int> _getDailyLimitMinutes(ChildSession session) async {
    try {
      final rulesDoc = await FirebaseFirestore.instance
          .collection('users')
          .doc(session.parentUid)
      ...
      if (data['applySameForAll'] == true) {
        ...
        limitHours = (entry?['limit'] ?? 2.0).toDouble();
      }
  ...
  Future<void> _ensureTasksForToday(ChildSession session) async {
    try {
      ...
      await _taskService.generateDailyTasks(
        ...
        childId: session.childId,
        childName: session.displayName,
        subjects: const ['Math', 'English', 'Science'],
        ageRange: '14-16',
      );
    }
  }
  ...
  Future<void> _clearInterceptionCooldown(String appName) async {
    try {
      ...
      print(
        'FocusPassWorkflowService: Cleared interception cooldown for $appName',
      );
    } catch (e) {
      print(
        'FocusPassWorkflowService: Error clearing interception cooldown: $e',
      );
    }
  }
...
class AppAccessResult {
  ...
  factory AppAccessResult.denied(String reason) {
    return AppAccessResult._(isAllowed: false, reason: reason);
  }
}
