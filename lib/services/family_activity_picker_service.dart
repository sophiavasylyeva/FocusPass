...
class FamilyActivityPickerResult {
  ...
  const FamilyActivityPickerResult({required this.appCount, required this.categoryCount});
...
class FamilyActivityPickerService {
  static const MethodChannel _channel = MethodChannel('com.focuspass.screentime');

  /// Presents the native picker. Returns null if the user cancelled.
  static Future<FamilyActivityPickerResult?> show() async {
    final result = await _channel.invokeMapMethod<String, dynamic>('showActivityPicker');
