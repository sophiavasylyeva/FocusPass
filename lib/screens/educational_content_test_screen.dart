...
class EducationalContentTestScreen extends StatefulWidget {
  ...
  const EducationalContentTestScreen({Key? key, required this.childName})
    : super(key: key);

  @override
  State<EducationalContentTestScreen> createState() =>
      _EducationalContentTestScreenState();
}

class _EducationalContentTestScreenState
    extends State<EducationalContentTestScreen> {
  ...
  Future<void> _initializeTasks() async {
    ...
    final existingTasks = await _taskService.fetchTasks(
    ...
    print(
      'EducationalContentTestScreen: Found ${existingTodayTasks.length} existing tasks for today',
    );

    // Only generate tasks if we don't have any for today
    if (existingTodayTasks.isEmpty) {
      print(
        'EducationalContentTestScreen: No tasks found for today, generating new tasks',
      );
      ...
      print(
        'EducationalContentTestScreen: Generating tasks for ${widget.childName} with subjects: $subjects',
      );
      ...
      print(
        'EducationalContentTestScreen: Using existing ${existingTodayTasks.length} tasks for today',
      );
    }
    ...
    final todaysPendingTasks = allTasks.where((task) {
      ...
      print(
        'EducationalContentTestScreen: Task ${task.id} - isToday: $isToday, isNotCompleted: $isNotCompleted, assignedAt: ${task.assignedAt}',
      );
    ...
    print(
      'EducationalContentTestScreen: Filtered to ${todaysPendingTasks.length} pending tasks for today',
    );
  ...
  Future<Map<String, dynamic>> _getChildData() async {
    try {
      final childRef = ChildSessionService.requireSession().childRef();
      final childDoc = await childRef.get();

      if (childDoc.exists) {
        final data = childDoc.data() ?? {};
        print(
          'EducationalContentTestScreen: Found child data at ${childRef.path}: $data',
        );
        return data;
      } else {
        print(
          'EducationalContentTestScreen: No child data found at ${childRef.path}',
        );
        return {};
      }
    } catch (e) {
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.pushAndRemoveUntil(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      ChildDashboardScreen(childName: widget.childName),
                ),
                (route) => false,
              );
            },
  ...
  Future<void> _refreshTasks() async {
    ...
    print(
      'EducationalContentTestScreen: _refreshTasks found ${todaysPendingTasks.length} pending tasks',
    );
