...
class _QuizScreenState extends State<QuizScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _celebrationSlideAnimation = Tween<double>(begin: 20.0, end: -10.0).animate(
  ...
  void _initializeQuiz() {
    ...
    for (
      int i = 0;
      i < widget.tasks.length && _questions.length < totalQuestions;
      i++
    ) {
                  ...
                  children: List.generate(_questions.length, (index) {
                    ...
                    if (_answeredQuestions.containsKey(index)) {
                      final wasCorrect =
                          ...
                          _questions[index].correctAnswerIndex;
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: List.generate(5, (i) {
                                  final delay = i * 0.15;
                                  final progress =
                                      (_celebrationFadeAnimation.value - delay)
                                          .clamp(0.0, 1.0);
  ...
  void _selectAnswer(int answerIndex) {
    ...
    setState(() {
      ...
      if (isCorrect) {
        ...
        _celebrationText =
            _celebrationMessages[Random().nextInt(_celebrationMessages.length)];
  ...
  Future<void> _retryWithNewQuestions() async {
    ...
    try {
      ...
      final subjects = List<String>.from(
        childData['subjectsOfInterest'] ?? ['Math'],
      );
      ...
      setState(() {
        ...
        for (
          int i = 0;
          i < todaysTasks.length && _questions.length < totalQuestions;
          i++
        ) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error generating new questions. Please try again.'),
        ),
      );
    }
  }
  ...
  Future<Map<String, dynamic>> _getChildData() async {
    try {
      final childDoc = await ChildSessionService.requireSession()
          .childRef()
          .get();
